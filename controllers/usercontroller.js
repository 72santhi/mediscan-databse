...
const signup = async (req, res) => {
  ...
  const saveFeedback = async (req, res) => {
    try {
      ...
      const { feedback, report, suggestions } = req.body;
      console.log(req.body);
      console.log(suggestions);
      // Save feedback to the database, assuming you have a Feedback model
      const newFeedback = await Feedback.create({ feedback,report,suggestions, });
  
      // Respond with a success message
      return res.status(200).json({ message: 'Feedback saved successfully', feedback: newFeedback });
    } catch (err) {
      console.error(err);
      return res.status(500).json({ message: 'Internal Server Error' });
    }
